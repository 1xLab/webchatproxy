...
(function () {
  ...
  async function handleChatRequest(msg) {
    try {
      ...
      let response = await waitForNewResponse(beforeCount);

      // Clean environment_details that the LLM may have echoed
      response = response.replace(/<environment_details>[\s\S]*?<\/environment_details>/g, '').trim();
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    ...
    if (msg.type === "cmd") {
      // Leitura de status em tempo real do DOM do ChatGPT
      const status = currentStatus();
      sendResponse({ ok: true, cmd: msg.cmd, url: window.location.href, ...status });
      return;
    }
  });

  // Le o estado atual do DOM: o que o assistant esta gerando (thinking/content)
  function currentStatus() {
    const streaming = isStreaming();
    const outgoing = document.querySelector('[data-message-author-role="user"]');
    let lastThought = "";
    let lastAnswer = "";
    const assistants = document.querySelectorAll('[data-message-author-role="assistant"]');
    if (assistants.length > 0) {
      const last = assistants[assistants.length - 1];
      // A IA costuma colocar o raciocinio/thinking em blocos "reasoning" antes da resposta
      const rh = last.querySelector('[class*="reasoning"], [data-testid="reasoning"], [class*="thought"]');
      if (rh) lastThought = (rh.textContent || "").replace(/[\u200B-\u200D\uFEFF]/g, "").trim();
      lastAnswer = getLatestAssistantText();
    }
    return {
      streaming,
      hasPendingUser: !!outgoing,
      latestThought: lastThought.slice(0, 3000),
      latestAnswer: lastAnswer.slice(0, 3000),
