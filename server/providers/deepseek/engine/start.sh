...
[ -d "$VENDOR_DIR/.git" ] || { echo "ERROR: DeepSeek vendor checkout metadata missing" >&2; exit 78; }
test "$(git -C "$VENDOR_DIR" rev-parse HEAD)" = "$PIN" || { echo "ERROR: DeepSeek vendor checkout is not at pinned commit $PIN" >&2; exit 78; }

export HOST="${DEEPSEEK_HOST:-127.0.0.1}"
export PORT="${DEEPSEEK_PORT:-3220}"
export DS_CDP="${DS_CDP:-http://127.0.0.1:9333}"
export DS_DATA_DIR="${DS_DATA_DIR:-$BASE_DIR/runtime/deepseek}"
export DS_AUTH_FILE="${DS_AUTH_FILE:-$DS_DATA_DIR/auth.json}"
export DS_SESSION_FILE="${DS_SESSION_FILE:-$DS_DATA_DIR/sessions.json}"
export DS_CHROME_PROFILE="${DS_CHROME_PROFILE:-$BASE_DIR/browser-profile-deepseek}"
export DS_SHOW_BROWSER="${DS_SHOW_BROWSER:-0}"

case "$HOST" in
  127.0.0.1|localhost|::1) ;;
  *)
    if [ -z "${DS_API_KEY:-}" ] && [ -z "${DS_API_KEY_FILE:-}" ]; then
      echo "ERROR: refusing non-loopback DeepSeek bind without DS_API_KEY or DS_API_KEY_FILE" >&2
      exit 78
    fi
    ;;
esac
