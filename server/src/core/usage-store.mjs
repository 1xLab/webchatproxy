...
export class UsageStore {
  ...
  async init() {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      try {
        ...
        if (!event?.job_id || this.jobIds.has(event.job_id)) continue;
        this.events.push(event);
        this.jobIds.add(event.job_id);
      } catch {}
    }
    return this;
  }
  ...
  async #estimate(provider, model, tokens, measurement) {
    ...
    const estimated = (
      uncached * inputRate
      + tokens.cached_input_tokens * cachedRate
      + tokens.output_tokens * outputRate
    ) / 1_000_000;
    return {
      estimated,
      currency: price.currency || 'USD',
      pricing_key: price.key || `${provider}:${model}`,
    };
  }
  ...
  summary(filters = {}) {
    ...
    for (const event of events) {
      ...
      const conversationKey = event.conversation_id || 'none';
      byProvider[providerKey] ??= emptyTotals();
      byModel[modelKey] ??= emptyTotals();
      byConversation[conversationKey] ??= emptyTotals();
      sum(byProvider[providerKey], event);
      sum(byModel[modelKey], event);
      sum(byConversation[conversationKey], event);
    }
