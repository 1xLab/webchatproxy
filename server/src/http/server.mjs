  ...
  return http.createServer(async(req,res)=>{
    try{
      ...
      if(req.method==='GET'&&url.pathname==='/v1/models'){
        const provider=facadeProvider||url.searchParams.get('provider'); if(!provider)return send(res,400,{error:{message:'provider is required'}});
      ...
      if(req.method==='POST'&&(url.pathname==='/v1/jobs'||url.pathname==='/v1/chat/completions')){
        ...
        if(facadeProvider&&incoming.provider&&String(incoming.provider).trim().toLowerCase()!==facadeProvider)return send(res,400,{error:{message:`provider is fixed by this port: ${facadeProvider}`,type:'invalid_request_error',code:'provider_port_mismatch'}});
