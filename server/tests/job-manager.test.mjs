...
class FakeBackend {
  constructor(){this.progress=new Map();this.page={url:()=>"https://chatgpt.com/c/test-conversation-123"};this.block=false;this.release=null;this.currentRequestId=null;this.context={close:async()=>{}};this.failOnce=false;this.startCalls=0;this.conversationIdValue="test-conversation-123";}
  progressOf(id){return this.progress.get(id)||null;}
  conversationId(){return this.conversationIdValue;}
  async start(){this.startCalls++;this.context={close:async()=>{}};this.page={url:()=>"https://chatgpt.com/c/test-conversation-123"};}
  async ask(messages,options){
    this.currentRequestId=options.requestId;
    if(this.failOnce){this.failOnce=false;throw new Error("Target page, context or browser has been closed");}
...
test("browser crash is recovered once automatically",async()=>{const f=await fixture();try{f.backend.failOnce=true;await f.manager.create({messages:[{role:"user",content:"recover"}],request_id:"recover-job"});const finished=await f.manager.waitFor("recover-job",2000);assert.equal(finished.status,"completed");assert.equal(finished.result.content,"reply:recover");assert.equal(f.backend.startCalls,1);assert.ok(f.journal.events.some(e=>e.event==="browser_auto_recovery"));}finally{await rm(f.runtimeDir,{recursive:true,force:true});}});
